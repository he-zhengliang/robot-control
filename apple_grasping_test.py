...
class GripperController:
    def __init__(self, port="/dev/ttyACM0", baudrate=9600, timeout=1):
        try:
            ...
            rospy.loginfo("[GripperController] Connected to gripper.")
        except serial.SerialException as e:
            rospy.logerr(f"[GripperController] Failed to connect: {e}")
            self.ser = None

    def send_hex_command(self, hex_string):
        if not self.ser:
            rospy.logwarn("[GripperController] Serial not initialized.")
            return
        try:
            cmd_bytes = bytearray.fromhex(hex_string)
            self.ser.write(cmd_bytes)
            time.sleep(0.1)
        except Exception as e:
            rospy.logwarn(f"[GripperController] Failed to send command: {e}")

    def close_hand(self):
        rospy.loginfo("[GripperController] Closing gripper and thumb...")
        self.send_hex_command(CLOSE_CMD)
        time.sleep(0.2)
        self.send_hex_command(THUMB_CLOSE_CMD)

    def open_hand(self):
        rospy.loginfo("[GripperController] Opening gripper and thumb...")
        self.send_hex_command(OPEN_CMD)
        time.sleep(0.2)
        self.send_hex_command(THUMB_OPEN_CMD)
...
def main():
    ...
    gripper = GripperController("/dev/ttyACM0")
        ...
        if current_point:
            ...
            if move_to_pose(group, approach_pose):
                ...
                if move_to_pose(group, target_pose):
                    rospy.loginfo("[pose_receiver] Reached target. Closing hand...")
                    gripper.close_hand()
                    rospy.sleep(6)
                    ...
                    if move_to_pose(group, approach_pose):
                        ...
                        if move_to_pose(group, specific_pose_1):
                            rospy.loginfo("[pose_receiver] Reached specific point 1. Opening hand...")
                            gripper.open_hand()
                            rospy.sleep(3)

                            # Step 5: Move to specific point 2
