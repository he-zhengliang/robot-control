        ...
        if current_point:
            ...
            if move_to_pose(group, approach_pose):
                ...
                if move_to_pose(group, target_pose):
                    rospy.loginfo("[pose_receiver] Reached target. Waiting 10 seconds...")
                    rospy.sleep(10)

                    # Step 3: Move back to approach point
                    rospy.loginfo("[pose_receiver] Moving back to approach point before next...")
                    if move_to_pose(group, approach_pose):
                        rospy.loginfo("[pose_receiver] Reached approach again. Waiting 1 second...")
                        rospy.sleep(0.5)
                        ...
                        specific_pose_1.pose.position.y = 0.32
                        specific_pose_1.pose.position.z = 0.25
                        q = quaternion_from_euler(0, np.pi/2, 0)
                        ...
                        if move_to_pose(group, specific_pose_1):
                            ...
                            specific_pose_2.pose.position.y = 0.05
                            specific_pose_2.pose.position.z = 0.3
                            specific_pose_2.pose.orientation = specific_pose_1.pose.orientation

                            if move_to_pose(group, specific_pose_2):
                                rospy.loginfo("[pose_receiver] Reached specific point 2. Waiting 5 seconds...")
                                rospy.sleep(5)
